...
export class MockAIProvider implements AIProvider {
  ...
  async generateAgentResponse(input: GenerateAgentResponseInput): Promise<AgentResponseResult> {
    ...
    const response = `${input.agentRole} counsel from ${input.agentName}: for this ${input.mode} decree, apply ${skills}. ${mockGuidance(input.agentRole)}${memory}${context} Royal command: "${input.command}"`;
