...
export const DEFAULT_SETTINGS: Array<{ key: string; value: string; category: SettingsCategory; description: string }> = [
  ...
  { key: "COUNCIL_AUTO_WORK_ORDER_MODE", value: "DRAFT", category: "SYSTEM", description: "Controls whether the Planner Agent auto-creates work orders after council completion. OFF = disabled; DRAFT = create draft work orders for King review; READY = create work orders ready for agent assignment." },
  ...
  { key: "PLANNER_CROSS_TASK_LEARNING", value: "true", category: "SYSTEM", description: "Let the planner learn from past outcomes: inject relevance-ranked lessons from similar prior work — what worked (PASS reviews) and what to avoid (failed reviews with a diagnosis) — into its planning context so it reuses wins and stops repeating past failures. Sourced from existing review records, outcome-gated, deterministic (no extra AI call). Default ON." },
  { key: "COUNCIL_CROSS_TASK_LEARNING", value: "true", category: "SYSTEM", description: "Same outcome lessons as PLANNER_CROSS_TASK_LEARNING, but injected into the council's shared memory context so every specialist agent AND the Grand Vizier synthesis deliberate with the relevant what-worked / what-to-avoid history — not just the post-hoc planner. Deterministic, no extra AI call, outcome-gated. Default ON." },
  ...
  { key: "SUPERVISED_AUTO_RETRY_ENABLED", value: "true", category: "SYSTEM", description: "When a runner job fails mechanically (PATCH_FAILED / VALIDATION_FAILED), let the Kingdom automatically re-dispatch a revision — carrying the reviewer's feedback — before escalating to the King. Conservative: LOW-priority work only, capped at the work order's maxAutoRetries, requires an online runner, results always land NEEDS_REVIEW, and nothing is pushed/merged/deployed. When exhausted, the King is notified. The King can always trigger a retry manually regardless of this setting." }
];
