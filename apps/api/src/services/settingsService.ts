...
export const DEFAULT_SETTINGS: Array<{ key: string; value: string; category: SettingsCategory; description: string }> = [
  ...
  { key: "COUNCIL_PARALLEL_SPECIALISTS", value: "false", category: "SYSTEM", description: "Run the council's specialist agents concurrently instead of one-by-one. Faster (the slowest specialist sets the pace instead of the sum), but changes how the council deliberates: specialists no longer see each other's answers (independent opinions), while the Grand Vizier still runs last with the full transcript and synthesizes. Default OFF keeps the sequential round-table where each agent builds on the prior. A/B the same decree before flipping." },
