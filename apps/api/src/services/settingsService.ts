...
export const DEFAULT_SETTINGS: Array<{ key: string; value: string; category: SettingsCategory; description: string }> = [
  ...
  { key: "AGENT_KNOWLEDGE_IN_CONTEXT", value: "false", category: "SYSTEM", description: "Inject each agent's APPROVED curated knowledge memories (the M16 Knowledge Lab lessons the King approved) into that agent's council prompt and the Grand Vizier synthesis — closing the loop where knowledge was created but never used during decree reasoning. Per-agent + project + global, ranked by use; deterministic. Default OFF." },
  { key: "CAPTURE_LESSONS_FROM_REVIEWS", value: "false", category: "SYSTEM", description: "When a runner job's review fails with a diagnosis (NEEDS_FIX / PATCH_FAILED / VALIDATION_FAILED + whatFailed), auto-propose a PENDING knowledge candidate capturing the lesson, attributed to the responsible agent. Once the King approves it (Knowledge Lab), it feeds back into council + planner via AGENT_KNOWLEDGE_IN_CONTEXT — making the learning loop self-sustaining. Candidates are King-gated; dedup is automatic. Default OFF." },
  { key: "CAPTURE_SUCCESSES_FROM_REVIEWS", value: "false", category: "SYSTEM", description: "When a runner job's review passes (PASS verdict + non-trivial summary + at least one whatPassed item), auto-propose a PENDING knowledge candidate capturing what worked, attributed to the responsible agent. Complements CAPTURE_LESSONS_FROM_REVIEWS: lessons from failure + lessons from success = a self-growing knowledge base. Unlike COUNCIL_SYNTHESIS_CAPTURE (which captured circular council-output), this captures real runner-execution evidence. Candidates are King-gated; dedup is automatic. Default OFF." },
