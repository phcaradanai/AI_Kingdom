 ...
 *   --with-context-repair (Priority 6):
 *   - LIVING_LOOP_AUTO_CONTEXT_REPAIR          → the loop auto-rebinds MISSING/STALE WorkOrder context (scans local
 *                                                 docs read-only + rebinds; the same safe action as the King's manual
 *                                                 "Refresh Context"). Off by default because it removes the FRESH-
 *                                                 context manual gate that previously sat in front of auto-patch.
 *
 * Usage (from repo root):
 *   npm run autonomy:enable                       # work-order mode READY, no auto sandbox patch, no auto context repair
 *   npm run autonomy:enable -- --mode=DRAFT        # work orders land as drafts for King review
 *   npm run autonomy:enable -- --with-sandbox-patch
 *   npm run autonomy:enable -- --with-context-repair
 *   npm run autonomy:disable                       # turn it all back off
 */

type Flags = { disable: boolean; mode: "OFF" | "DRAFT" | "READY"; withSandboxPatch: boolean; withContextRepair: boolean };

function parseFlags(argv: string[]): Flags {
  const flags: Flags = { disable: false, mode: "READY", withSandboxPatch: false, withContextRepair: false };
  for (const arg of argv) {
    if (arg === "--disable") flags.disable = true;
    else if (arg === "--with-sandbox-patch") flags.withSandboxPatch = true;
    else if (arg === "--with-context-repair") flags.withContextRepair = true;
...
async function main(): Promise<void> {
  ...
  if (flags.disable) {
    ...
    await setSetting("LIVING_LOOP_AUTO_CONTEXT_REPAIR", "false", "Auto-rebind MISSING/STALE WorkOrder context.");
  ...
  await setSetting(
    "LIVING_LOOP_AUTO_CONTEXT_REPAIR",
    flags.withContextRepair ? "true" : "false",
    "Auto-rebind MISSING/STALE WorkOrder context (read-only scan + rebind; no push/PR/merge/deploy)."
  );
  ...
  if (flags.withContextRepair) {
    console.log("  • Auto context repair ON: the loop rebinds MISSING/STALE WorkOrder context itself (read-only scan + rebind). This removes the manual 'Refresh Context' gate that previously sat before auto-patch.");
  }
  if (flags.mode === "READY") {
