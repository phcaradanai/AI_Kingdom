...
import { AGENT_INSTALL_HINTS, EXECUTION_MODES, EXTERNAL_AGENT_TYPES, SAFETY_LEVELS, type AgentInstallHint } from "./externalAgentModels";
import type { ExternalAgentType } from "@/types/api";
...
function InstallHintCallout({ hint, autoFilled, tk }: { hint: AgentInstallHint | undefined; autoFilled: boolean; tk: (key: string) => string }) {
  if (!hint) return null;
  return <div className="space-y-2 rounded-md border border-amber-500/20 bg-amber-500/5 px-3 py-2.5 text-xs leading-5">
    <div className="font-semibold text-amber-200">{tk("externalAgents.install.title")}</div>
    {autoFilled ? <p className="text-muted-foreground">{tk("externalAgents.install.autoFilled")}</p> : null}
    <div className="space-y-1">
      <Row label={tk("externalAgents.install.command")} value={hint.installCommand} mono />
      <Row label={tk("externalAgents.install.check")} value={hint.checkCommand} mono />
      <Row label={tk("externalAgents.install.runnerEnv")} value={hint.runnerEnv} mono multiline />
      {hint.note ? <Row label={tk("externalAgents.install.note")} value={hint.note} /> : null}
    </div>
...
function Row({ label, value, mono = false, multiline = false }: { label: string; value: string; mono?: boolean; multiline?: boolean }) {
  return <div className="flex flex-wrap gap-1.5"><span className="shrink-0 text-muted-foreground">{label}:</span>{multiline ? <pre className="whitespace-pre-wrap font-mono text-foreground">{value}</pre> : <span className={mono ? "font-mono text-foreground" : "text-foreground"}>{value}</span>}</div>;
}
