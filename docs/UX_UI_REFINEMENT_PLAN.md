...
Progress: **Waves 4A-4I complete.** `/agents`, `/external-agents`, `/agent-chat`, `/living-agents`, `/living-agents/:agentId`, `/providers`, `/routing`, `/treasury`, and `/usage-traces/:traceId` now use compact source-linked workspaces, semantic English/Thai chrome, responsive focused handoff, and preserved routing, readiness, profile, presence, direct-conversation, provider registry, secret-safety, financial evidence, save, and review contracts.
...
Wave 4I implementation replaces the 497-line Usage Trace detail page with a 470-line evidence-first audit page. A compact attribution summary (trace ID, purpose, attribution badge, actor, provider, model, timing) anchors the first viewport; a token/cost strip and Final Resolution card follow with fallback/sandbox-after-API explanations. An ordered operation timeline uses colored step-type badges (`PROVIDER_CALL`, `PROVIDER_CALL_SUCCESS`, `PROVIDER_CALL_FAILED`, `PROVIDER_FALLBACK`, `AGENT_RESPONSE`, `FINAL_COUNSEL`, and six more) with expandable safe previews on each step. Related Records links (Task, Council, Agent, Reports, Project) and Source Ownership links (Provider Config, Route Chain, Treasury, Audit) appear in a two-column grid below the timeline. Sanitized prompt/response previews with recovery message when null close the page, followed by the full usage-record table. Phantom `costSource` dead logic removed (field is never selected in the Prisma query). Source-of-truth separation preserved: no backend, DTO, Prisma, routing, secret handling, or provider behavior changed. Scoped English/Thai chrome via `usageTraceMessages.ts` registered in `i18nMessages.ts`. 44px controls on the expand toggle and back button; no horizontal overflow at any breakpoint. Focused web tests: 11 scenarios covering normal trace (all sections), related record links, fallback count and sandbox-after-API warning, sandbox-no-API warning, failed trace with error, legacy trace (no timeline steps, recovery message), null previews (two recovery messages), preview visible when present, step preview expand toggle, source ownership hrefs, EN labels, and TH labels via localStorage. Verification: 260/260 web tests, root typecheck, production build, and `git diff --check` pass. Rendered English/Thai checks at 1440x900 and 390x844 pass with no horizontal overflow, all source-ownership links present, and proper Thai rendering for all `trace.*` keys.
