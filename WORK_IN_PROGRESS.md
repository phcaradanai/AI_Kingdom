...
- **Prove + measure + prune (King chose this 2026-06-25). Phase 2 DONE: live verification** — ran 1 real-provider decree on project with approved knowledge (2026-06-25). Results: fallbackNotice=none (0% fallback — **43% was fully historical**), provider=OpenRouter/deepseek-v4-flash (paid, real AI, not mock), cost=$0.004067, useCount delta +18 (from 0 to 18, confirmed AGENT_KNOWLEDGE_IN_CONTEXT works). Only 3 of 19 approved knowledge items consumed — other 16 are project-scoped to different projects, not a bug, just silo effect. **Phase 3 (next): knowledge silo gap** — 16 of 19 approved items permanently zero because project-scoped. Fix: promote high-confidence lessons to global scope (agentId=null, projectId=null) OR allow cross-project knowledge query. Also: 10 PENDING candidates need King review to enter the loop.
